...
I love the look, but I sometimes feel that this aesthetic has a somewhat limited imagination. Just because the technology is modern doesn't mean the *object* has to look modern. In fact, I think technology has progressed to the point where we can start looking *back* for inspiration. 
...
![Celadon green without the stand-in caps, viewed from the front.]({{ site.url }}/images/adam_ic/green_nokeys_front2.png)
...
![Front of the version with the profile of Robert Adam, from a different angle]({{ site.url }}/images/adam_ic/green_keys_face_front.png)
...
![A bust of Joan of Arc.]({{ site.url }}/images/adam_ic/joain.png)
...
**ADAM** is a split ortholinear porcelain keyboard case. It will be compatible with the Let's Split v2 PCB, and will have handwired versions available as well. The constraints of working with porcelain necessitate designing a special plate, so **ADAM** will not be compatible with previous Let's Split plates.  

Unlike [Adrien Friggeri's new 'Blanck' keyboard](http://medium.com/@friggeri/the-new-blanck-keyboard-c7563c4f9fa) (which admittedly started this whole endeavour in the first place), **ADAM** will be designed so that the only personal assembly users will need to do is to glue in M3 standoffs and screw in the top plate. The renders do not yet include the mounting slots for the standoffs---I'm still finalizing the exact specifications for these.

I haven't set prices yet, but each half will be *at least* \$100. If there is a lot of interest, I might be able to work out a deal with the manufacturers I am partnering with (doubtful though), but I'm guessing a full keyboard set (plates included) is going to run for about \$275-\$300.  
...
[^1]: "Actual" kintsugi uses lacquer as a joiner, but there are loads of videos online on how to achieve similar effects by yourself with tools like super-glue.
